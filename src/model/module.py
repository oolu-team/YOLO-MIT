...
from src.utils.module_utils import auto_pad, create_activation_function, round_up
...
class Anchor2Vec(nn.Module):
    def __init__(self, reg_max: int = 16) -> None:
        super().__init__()
        reverse_reg = torch.arange(reg_max, dtype=torch.float32).view(
            1, reg_max, 1, 1, 1
        )
        self.anc2vec = nn.Conv3d(
            in_channels=reg_max, out_channels=1, kernel_size=1, bias=False
        )
...
class Classification(nn.Module):
    def __init__(
        self, in_channel: int, num_classes: int, *, neck_channels=1024, **head_args
    ):
        super().__init__()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bottleneck = nn.Sequential(
            *[
                Bottleneck(neck_channels, neck_channels, **neck_args)
...
class ELAN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv4 = Conv(
            part_channels + 2 * process_channels, out_channels, 1, **kwargs
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv3 = nn.Sequential(
            RepNCSP(
                process_channels, process_channels, neck_args=csp_neck_args, **csp_args
            ),
            Conv(process_channels, process_channels, 3, padding=1, **kwargs),
        )
        self.conv4 = Conv(
...
class SPPCSPConv(nn.Module):
    ...
    def __init__(
        ...
        self.pools = nn.ModuleList(
            [Pool(kernel_size=kernel_size, stride=1) for kernel_size in kernel_sizes]
        )
        self.post_conv = nn.Sequential(
            Conv(4 * neck_channels, neck_channels, 1),
            Conv(neck_channels, neck_channels, 3),
        )
